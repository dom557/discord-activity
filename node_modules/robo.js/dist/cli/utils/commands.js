...
function buildSlashCommands(dev, commands) {
  ...
  return Object.entries(commands).map(([key, entry]) => {
    ...
    let commandBuilder;
    try {
      commandBuilder = new SlashCommandBuilder().setName(key).setNameLocalizations(entry.nameLocalizations || {}).setDescription(entry.description || "No description provided").setDescriptionLocalizations(entry.descriptionLocalizations || {});
    } catch (e) {
...
async function registerCommands(dev, force, newCommands, newMessageContextCommands, newUserContextCommands, changedCommands, addedCommands, removedCommands, changedContextCommands, addedContextCommands, removedContextCommands) {
  ...
  try {
    ...
    if (force) {
      const deletions = existingCommands.map((command) => {
        return rest.delete(
          guildId ? Routes.applicationGuildCommand(clientId, guildId, command.id) : Routes.applicationCommand(clientId, command.id)
        );
      });
      ...
      const deletions = removedCommands.map((command) => {
        ...
        if (existingCommand) {
          return rest.delete(
            guildId ? Routes.applicationGuildCommand(clientId, guildId, existingCommand.id) : Routes.applicationCommand(clientId, existingCommand.id)
          );
        }
      });
      await Promise.all(deletions);
